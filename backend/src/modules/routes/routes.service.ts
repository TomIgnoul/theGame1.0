...
import { findByTheme } from '../gems/gems.repo';
import { computeWalkingRoute } from './routing.provider';
...
export async function generateRoute(req: RouteRequest): Promise<RouteResponse> {
  const { theme, kmTarget, shape, start, end } = req;

  if (kmTarget < KM_MIN || kmTarget > KM_MAX) {
    throw { status: 400, message: `kmTarget must be between ${KM_MIN} and ${KM_MAX}` };
  }
  ...
  const tolerance = (ROUTE_TOLERANCE_PERCENT / 100) * targetMeters;
  ...
  for (let attempt = 0; attempt <= ROUTE_MAX_RETRIES; attempt++) {
    const n = Math.min(
      MAX_GEMS,
      Math.max(MIN_GEMS, Math.round(DEFAULT_GEMS + (attempt - 1) * 2))
    );
    const selected = selectGemsByDistance(
      gemCandidates,
      start,
      shape === 'a_to_b' ? end! : null,
      n
    );
    ...
    try {
      const result = await computeWalkingRoute(origin, destination, waypoints);
