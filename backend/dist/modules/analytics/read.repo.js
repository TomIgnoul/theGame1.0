...
exports.fetchAnalyticsTimeseries = fetchAnalyticsTimeseries;
exports.fetchAnalyticsThemeBreakdown = fetchAnalyticsThemeBreakdown;
exports.fetchAnalyticsPoiBreakdown = fetchAnalyticsPoiBreakdown;
const db_1 = require("../../db");
async function fetchAnalyticsOverview(filters) {
    const result = await db_1.pool.query(`SELECT
     ...
     WHERE e.occurred_at >= $1::date
       ...
       AND ($3::text IS NULL OR COALESCE(e.theme, g.theme) = $3)`, [filters.from, filters.to, filters.theme]);
...
async function fetchAnalyticsTimeseries(filters) {
    const result = await db_1.pool.query(`WITH days AS (
     ...
     ORDER BY days.bucket_date ASC`, [filters.from, filters.to, filters.theme]);
...
async function fetchAnalyticsThemeBreakdown(filters) {
    const result = await db_1.pool.query(`SELECT
     ...
     WHERE e.occurred_at >= $1::date
       ...
       AND ($3::text IS NULL OR COALESCE(e.theme, g.theme) = $3)
     GROUP BY 1
     ORDER BY total_events DESC, theme ASC NULLS LAST`, [filters.from, filters.to, filters.theme]);
    return result.rows.map((row) => ({
        theme: row.theme,
...
async function fetchAnalyticsPoiBreakdown(filters) {
    const result = await db_1.pool.query(`SELECT
     ...
     WHERE e.poi_id IS NOT NULL
       ...
       AND ($3::text IS NULL OR COALESCE(e.theme, g.theme) = $3)
     GROUP BY e.poi_id
     ORDER BY total_events DESC, title ASC NULLS LAST, e.poi_id ASC`, [filters.from, filters.to, filters.theme]);
    return result.rows.map((row) => ({
        poiId: row.poi_id,
...
//# sourceMappingURL=read.repo.js.map