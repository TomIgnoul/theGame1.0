...
| FR-27 | Offer an admin-only Add Pearl form to an already logged-in admin | `GH-PRL-01`<br>`GH-PRL-03`<br>`GH-PRL-05` | `3 5 UI Design`<br>`3 6 UI ↔ API Mapping` | `backend/src/app.ts`<br>`backend/src/modules/admin/pearls.service.ts`<br>`frontend/src/routes/AdminAddPearlPage.tsx`<br>`frontend/src/components/admin/AddPearlForm.tsx`<br>`frontend/src/components/admin/AdminDashboardPage.tsx` | `TC-PRL-SEC-01`<br>`TC-PRL-UI-01`<br>`TC-PRL-UI-02` | VERIFIED | Admin Add Pearl route and form exist behind the existing admin-session pattern, are linked from the admin dashboard, and are covered by UI/API auth and happy-path tests |
| FR-28 | Validate required Add Pearl fields: name, story, theme, latitude, longitude, and PearlOwner | `GH-PRL-01`<br>`GH-PRL-03` | `3 5 UI Design`<br>`3 6 UI ↔ API Mapping`<br>`3 7 API Contracts` | `backend/src/app.ts`<br>`backend/src/modules/admin/pearls.service.ts`<br>`frontend/src/components/admin/AddPearlForm.tsx` | `TC-PRL-API-02`<br>`TC-PRL-UI-02`<br>`TC-PRL-UI-04` | VERIFIED | Backend and frontend validation reject missing/invalid fields and surface backend validation errors; admin-created Pearls use coordinates as the required location input |
| FR-29 | Restrict manually added Pearl themes to War, Museum, Streetart, Food, and Culture | `GH-PRL-03`<br>`GH-PRL-04` | `2 Specificaties`<br>`3 2 Database Design (PostgreSQL)`<br>`3 7 API Contracts` | `backend/src/modules/admin/pearls.service.ts`<br>`frontend/src/constants.ts`<br>`frontend/src/components/admin/AddPearlForm.tsx` | `TC-PRL-API-02`<br>`TC-PRL-DB-01`<br>`TC-PRL-UI-02` | VERIFIED | Backend and frontend Add Pearl paths use the PRL whitelist only |
| FR-30 | Model PearlOwner as a real relationship where each Pearl belongs to exactly one PearlOwner | `GH-PRL-01`<br>`GH-PRL-02`<br>`GH-PRL-03` | `3 1 Architectuur`<br>`3 2 Database Design (PostgreSQL)`<br>`3 6 UI ↔ API Mapping`<br>`3 7 API Contracts` | `db/migrations/003_pearl_owners.sql`<br>`backend/src/db/schema.sql`<br>`backend/src/modules/admin/pearls.service.ts`<br>`frontend/src/components/admin/PearlOwnerSelect.tsx` | `TC-PRL-DB-01`<br>`TC-PRL-API-01`<br>`TC-PRL-API-03`<br>`TC-PRL-UI-03` | VERIFIED | DB, backend, and frontend select/create flow enforce PearlOwner as a relationship |
| FR-31 | Make a newly saved Pearl available to the existing route generation flow | `GH-PRL-01`<br>`GH-PRL-04` | `3 1 Architectuur`<br>`3 2 Database Design (PostgreSQL)`<br>`3 7 API Contracts` | `backend/src/modules/admin/pearls.service.ts`<br>`backend/src/modules/gems/gems.repo.ts`<br>`backend/src/modules/routes/routes.service.ts`<br>`frontend/src/routes/AdminAddPearlPage.tsx` | `TC-PRL-API-04`<br>`TC-PRL-INT-01`<br>`TC-PRL-UI-01` | VERIFIED | Backend saves Pearls as active manual `gems`; automated integration coverage proves the route-service candidate loader can include the newly created Pearl for the same theme |
...
| NFR-S7 | Admin Pearl write access | `2 Specificaties`<br>`3 1 Architectuur`<br>`3 5 UI Design`<br>`3 6 UI ↔ API Mapping`<br>`3 7 API Contracts` | `backend/src/app.ts`<br>`backend/src/modules/admin/auth.ts`<br>`backend/src/modules/admin/pearls.service.ts`<br>`frontend/src/routes/AdminAddPearlPage.tsx` | `TC-PRL-SEC-01`<br>`TC-PRL-API-01` | VERIFIED | Linked GH: `GH-PRL-05`. Backend and frontend Add Pearl paths require the existing admin session without adding login/account implementation |
...
| `GH-PRL-01` Admin adds a Pearl with an existing PearlOwner | FR-27, FR-28, FR-30, FR-31 | Backend Add Pearl API create flow, PearlOwner relationship, frontend form, and route candidate inclusion are covered | `TC-PRL-API-03`<br>`TC-PRL-API-04`<br>`TC-PRL-UI-01`<br>`TC-PRL-INT-01` | VERIFIED | Save flow and route-candidate inclusion are covered without adding a separate Pearl routing path |
| `GH-PRL-02` Admin creates a PearlOwner during Add Pearl | FR-30 | PearlOwner create API and frontend select/create flow exist | `TC-PRL-API-01`<br>`TC-PRL-UI-03` | VERIFIED | New owner can be created and selected before saving a Pearl |
| `GH-PRL-03` Invalid or missing Pearl fields block save | FR-27, FR-28, FR-29, FR-30 | Backend and frontend validation reject missing fields, invalid theme, invalid coordinates, and invalid PearlOwner IDs | `TC-PRL-API-02`<br>`TC-PRL-UI-02`<br>`TC-PRL-UI-04` | VERIFIED | Invalid inputs are blocked and backend validation errors are rendered cleanly |
| `GH-PRL-04` New Pearl is selectable for route generation | FR-29, FR-31 | Backend persists Pearls as active manual `gems`; the existing route-service candidate loader reads active `gems` by theme; frontend success state confirms route-candidate status | `TC-PRL-API-04`<br>`TC-PRL-INT-01`<br>`TC-PRL-UI-01` | VERIFIED | Automated proof covers route-candidate eligibility; live Google Directions/map smoke remains manual environment validation |
| `GH-PRL-05` Add Pearl remains admin-only | FR-27 | Backend Add Pearl endpoints and frontend Add Pearl route require the existing admin session | `TC-PRL-SEC-01` | VERIFIED | Admin-only scope is enforced without adding login/account implementation |
...
| `pearl_owners` | `3 2 Database Design` | FR-30<br>NFR-S7 | `db/migrations/003_pearl_owners.sql`<br>`backend/src/db/schema.sql`<br>`backend/src/modules/admin/pearls.service.ts` | `TC-PRL-DB-01`<br>`TC-PRL-API-01` | IMPLEMENTED | PearlOwner storage now exists and backs the admin-only lookup/create API |
| `gems` | `3 2 Database Design` | FR-01 to FR-06, FR-14, FR-27 to FR-31 | `backend/src/db/schema.sql`<br>`backend/src/modules/gems/gems.repo.ts`<br>`db/migrations/003_pearl_owners.sql`<br>`backend/src/modules/admin/pearls.service.ts` | Manual API review<br>`TC-PRL-DB-01`<br>`TC-PRL-API-04`<br>`TC-PRL-INT-01` | PARTIAL | Baseline gems exist; PRL-00 now adds manual Pearl owner linkage and active route-candidate persistence, while broader route UX gaps remain |
...
| `/api/admin/pearl-owners` | `GET` | `3 7 API Contracts` | FR-27, FR-30<br>NFR-S7 | `backend/src/app.ts`<br>`backend/src/modules/admin/pearls.service.ts` | `TC-PRL-API-01`<br>`TC-PRL-SEC-01` | IMPLEMENTED | Admin-only lookup for selecting an existing PearlOwner |
| `/api/admin/pearl-owners` | `POST` | `3 7 API Contracts` | FR-30<br>NFR-S7 | `backend/src/app.ts`<br>`backend/src/modules/admin/pearls.service.ts` | `TC-PRL-API-01`<br>`TC-PRL-SEC-01` | IMPLEMENTED | Admin-only create flow for PearlOwner records, including duplicate-owner 409 handling |
| `/api/admin/pearls` | `POST` | `3 7 API Contracts` | FR-27, FR-28, FR-29, FR-30, FR-31<br>NFR-S7 | `backend/src/app.ts`<br>`backend/src/modules/admin/pearls.service.ts`<br>`backend/src/modules/gems/gems.repo.ts` | `TC-PRL-API-02`<br>`TC-PRL-API-03`<br>`TC-PRL-API-04`<br>`TC-PRL-SEC-01` | IMPLEMENTED | Admin-only create endpoint validates required fields, links PearlOwner, and persists active manual route candidates |
...
| `frontend/src/components/admin/AdminDashboardPage.tsx` | `3 5 UI Design` | FR-20, FR-21, FR-22, FR-23, FR-24, FR-26, FR-27 | `/api/admin/auth/logout`, `/api/admin/analytics/overview`, `/api/admin/analytics/timeseries`, `/api/admin/analytics/breakdowns` | `npm run test -w frontend` | IMPLEMENTED | Dashboard shell coordinates filter, loading, empty, error, success states, and the Add Pearl navigation link |
...
| `frontend/src/routes/AdminAddPearlPage.tsx` | `3 5 UI Design`<br>`3 6 UI ↔ API Mapping` | FR-27, FR-28, FR-29, FR-30, FR-31 | `/api/admin/pearl-owners`, `/api/admin/pearls` | `TC-PRL-UI-01`<br>`TC-PRL-UI-02`<br>`TC-PRL-SEC-01` | IMPLEMENTED | Admin-only route/shell for Add Pearl uses the existing admin-session pattern; login implementation remains out of scope |
| `frontend/src/components/admin/AddPearlForm.tsx` | `3 5 UI Design`<br>`3 6 UI ↔ API Mapping` | FR-27, FR-28, FR-29, FR-31 | `/api/admin/pearls` | `TC-PRL-UI-01`<br>`TC-PRL-UI-02`<br>`TC-PRL-UI-04` | IMPLEMENTED | Required field, PRL theme, coordinate, submit, backend-error, and success states are covered |
| `frontend/src/components/admin/PearlOwnerSelect.tsx` | `3 5 UI Design`<br>`3 6 UI ↔ API Mapping` | FR-30 | `/api/admin/pearl-owners` | `TC-PRL-UI-03` | IMPLEMENTED | Select-or-create UI persists PearlOwner as a relationship target before Pearl save |
...
## 14. Admin Pearl Management Test Cases

These IDs track PRL-00 implementation evidence. Backend foundation, frontend Add Pearl UI, and route-candidate inclusion evidence are now present; live Google Directions/map smoke remains manual environment validation.

| Test Case ID | Coverage Summary | Evidence | Linked FR / NFR / GH |
| --- | --- | --- | --- |
| `TC-PRL-DB-01` | PearlOwner schema and manual Pearl relationship are migrated with required constraints | `db/migrations/003_pearl_owners.sql`<br>`backend/src/db/schema.sql` review | FR-29, FR-30 |
| `TC-PRL-API-01` | Admin can search/create PearlOwner records and non-admin requests are blocked | `backend/src/app.admin.pearls.test.ts` -> `GET /api/admin/pearl-owners returns PearlOwners for an authorized admin`; `POST /api/admin/pearl-owners creates a PearlOwner for an authorized admin`; `admin Pearl endpoints block requests without an admin session`; `POST /api/admin/pearl-owners returns 409 for an existing PearlOwner` | FR-30, NFR-S7, `GH-PRL-02`, `GH-PRL-05` |
| `TC-PRL-API-02` | Add Pearl rejects missing required fields, invalid coordinates, and themes outside the MVP whitelist | `backend/src/app.admin.pearls.test.ts` -> `POST /api/admin/pearls rejects missing required fields before saving`; `POST /api/admin/pearls rejects themes outside the PRL-00 whitelist`; `POST /api/admin/pearls rejects invalid latitude and longitude values` | FR-28, FR-29, `GH-PRL-03` |
| `TC-PRL-API-03` | Add Pearl saves a valid Pearl linked to exactly one existing PearlOwner | `backend/src/app.admin.pearls.test.ts` -> `POST /api/admin/pearls creates an active route-eligible Pearl with an existing PearlOwner`; `POST /api/admin/pearls returns 404 when the PearlOwner does not exist` | FR-27, FR-28, FR-30, `GH-PRL-01` |
| `TC-PRL-API-04` | Add Pearl response marks the new Pearl as an active route candidate | `backend/src/app.admin.pearls.test.ts` -> `POST /api/admin/pearls creates an active route-eligible Pearl with an existing PearlOwner` | FR-31, `GH-PRL-04` |
| `TC-PRL-INT-01` | Existing route generation candidate loading can include the newly created active manual Pearl | `backend/src/modules/admin/pearls.integration.test.ts` -> `admin-created Pearl is persisted as one active manual gem and appears in route candidates`; `backend/src/modules/routes/routes.service.ts` -> `loadRouteCandidateGems` uses the shared active `gems` query by theme | FR-31, `GH-PRL-01`, `GH-PRL-04` |
| `TC-PRL-UI-01` | Authenticated admin can complete the Add Pearl happy path with an existing PearlOwner | `frontend/src/routes/AdminAddPearlPage.test.tsx` -> `submits a Pearl with an existing PearlOwner and shows success` | FR-27, FR-28, FR-31, `GH-PRL-01` |
| `TC-PRL-UI-02` | Add Pearl form shows inline validation for missing fields, invalid coordinates, and invalid theme coverage | `frontend/src/routes/AdminAddPearlPage.test.tsx` -> `shows validation errors for missing required fields`; `shows validation errors for invalid coordinates` | FR-28, FR-29, `GH-PRL-03` |
| `TC-PRL-UI-03` | Add Pearl form supports selecting an existing PearlOwner and creating a new one | `frontend/src/routes/AdminAddPearlPage.test.tsx` -> `renders the Add Pearl form and loads existing PearlOwners`; `creates and selects a new PearlOwner before saving` | FR-30, `GH-PRL-02` |
| `TC-PRL-UI-04` | Add Pearl form shows backend API validation errors cleanly | `frontend/src/routes/AdminAddPearlPage.test.tsx` -> `shows backend API errors cleanly` | FR-28, FR-29, `GH-PRL-03` |
| `TC-PRL-SEC-01` | Add Pearl UI/API remain inaccessible without a valid admin session | `backend/src/app.admin.pearls.test.ts` -> `admin Pearl endpoints block requests without an admin session`; `frontend/src/routes/AdminAddPearlPage.test.tsx` -> `shows the admin login screen when the session is unauthorized` | FR-27, NFR-S7, `GH-PRL-05` |
| `TC-PRL-DOC-01` | PRL-00 requirements, Gherkin scenarios, design docs, backlog split, and traceability mappings remain aligned | `docs/traceability.md`; source design docs review | FR-27 to FR-31, NFR-S7 |
