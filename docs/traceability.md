...
| FR-27 | Offer an admin-only Add Pearl form to an already logged-in admin | `GH-PRL-01`<br>`GH-PRL-03`<br>`GH-PRL-05` | `3 5 UI Design`<br>`3 6 UI ↔ API Mapping` | Planned: `frontend/src/routes/AdminAddPearlPage.tsx`<br>`frontend/src/components/admin/AddPearlForm.tsx` | Planned: `TC-PRL-UI-01`<br>`TC-PRL-UI-02`<br>`TC-PRL-SEC-01` | NOT STARTED | PRL-00 is docs-only in this branch; no login/account implementation is added |
| FR-28 | Validate required Add Pearl fields: name, story, address, theme, latitude, longitude, and PearlOwner | `GH-PRL-01`<br>`GH-PRL-03` | `3 5 UI Design`<br>`3 6 UI ↔ API Mapping`<br>`3 7 API Contracts` | Planned: `backend/src/app.ts`<br>`backend/src/modules/admin/pearls.service.ts`<br>`frontend/src/components/admin/AddPearlForm.tsx` | Planned: `TC-PRL-API-02`<br>`TC-PRL-UI-02` | NOT STARTED | Validation must reject missing required fields before any Pearl is saved |
| FR-29 | Restrict manually added Pearl themes to War, Museum, Streetart, Food, and Culture | `GH-PRL-03`<br>`GH-PRL-04` | `2 Specificaties`<br>`3 2 Database Design (PostgreSQL)`<br>`3 7 API Contracts` | Planned: `backend/src/config/constants.ts`<br>`backend/src/modules/admin/pearls.service.ts`<br>`frontend/src/constants.ts` | Planned: `TC-PRL-API-02`<br>`TC-PRL-DB-01`<br>`TC-PRL-UI-02` | NOT STARTED | Must use the existing MVP theme whitelist only |
| FR-30 | Model PearlOwner as a real relationship where each Pearl belongs to exactly one PearlOwner | `GH-PRL-01`<br>`GH-PRL-02`<br>`GH-PRL-03` | `3 1 Architectuur`<br>`3 2 Database Design (PostgreSQL)`<br>`3 6 UI ↔ API Mapping`<br>`3 7 API Contracts` | Planned: `db/migrations/00X_pearl_owners.sql`<br>`backend/src/modules/admin/pearlOwners.service.ts`<br>`backend/src/modules/admin/pearls.service.ts` | Planned: `TC-PRL-DB-01`<br>`TC-PRL-API-01`<br>`TC-PRL-API-03` | NOT STARTED | No loose owner text field; manual Pearls must reference a PearlOwner row |
| FR-31 | Make a newly saved Pearl available to the existing route generation flow | `GH-PRL-01`<br>`GH-PRL-04` | `3 1 Architectuur`<br>`3 2 Database Design (PostgreSQL)`<br>`3 7 API Contracts` | Planned: `backend/src/modules/gems/gems.repo.ts`<br>`backend/src/modules/routes/routes.service.ts`<br>`backend/src/modules/admin/pearls.service.ts` | Planned: `TC-PRL-API-04`<br>`TC-PRL-INT-01` | NOT STARTED | Saved Pearls should be active manual `gems` and selectable by the existing candidate query |
...
| NFR-S7 | Admin Pearl write access | `2 Specificaties`<br>`3 1 Architectuur`<br>`3 5 UI Design`<br>`3 6 UI ↔ API Mapping`<br>`3 7 API Contracts` | Planned: `backend/src/modules/admin/auth.ts`<br>`backend/src/modules/admin/pearls.service.ts`<br>`frontend/src/routes/AdminAddPearlPage.tsx` | Planned: `TC-PRL-SEC-01`<br>`TC-PRL-API-01` | NOT STARTED | Linked GH: `GH-PRL-05`. Add Pearl UI/API must require an existing admin session and must not introduce a new login/account flow |
...
| `GH-PRL-01` Admin adds a Pearl with an existing PearlOwner | FR-27, FR-28, FR-30, FR-31 | Planned Add Pearl form, API create flow, PearlOwner relationship, and route candidate availability | Planned: `TC-PRL-API-03`<br>`TC-PRL-UI-01`<br>`TC-PRL-INT-01` | NOT STARTED | PRL-00 governance baseline only; implementation should prove save + route eligibility |
| `GH-PRL-02` Admin creates a PearlOwner during Add Pearl | FR-30 | Planned PearlOwner create API and UI select/create flow | Planned: `TC-PRL-API-01`<br>`TC-PRL-UI-03` | NOT STARTED | New owner must be persisted as a relationship target before linking the Pearl |
| `GH-PRL-03` Invalid or missing Pearl fields block save | FR-27, FR-28, FR-29, FR-30 | Planned backend validation and frontend inline validation states | Planned: `TC-PRL-API-02`<br>`TC-PRL-UI-02` | NOT STARTED | Must reject invalid theme and must not save loose PearlOwner text |
| `GH-PRL-04` New Pearl is selectable for route generation | FR-29, FR-31 | Planned active manual `gems` persistence and existing route candidate query integration | Planned: `TC-PRL-API-04`<br>`TC-PRL-INT-01` | NOT STARTED | Route generation can include the Pearl when the route algorithm selects matching candidates |
| `GH-PRL-05` Add Pearl remains admin-only | FR-27 | Planned admin-session guard for UI and API | Planned: `TC-PRL-SEC-01` | NOT STARTED | This scenario enforces admin-only scope without adding login/account implementation |
...
| `pearl_owners` | `3 2 Database Design` | FR-30<br>NFR-S7 | Planned: `db/migrations/00X_pearl_owners.sql`<br>`backend/src/modules/admin/pearlOwners.service.ts` | Planned: `TC-PRL-DB-01`<br>`TC-PRL-API-01` | NOT STARTED | Required for PRL-00 so PearlOwner is a real relationship rather than a loose text field |
| `gems` | `3 2 Database Design` | FR-01 to FR-06, FR-14, FR-27 to FR-31 | `backend/src/db/schema.sql`<br>`backend/src/modules/gems/gems.repo.ts`<br>Planned: `db/migrations/00X_pearl_owners.sql` | Manual API review<br>Planned: `TC-PRL-DB-01`<br>`TC-PRL-INT-01` | PARTIAL | Baseline gems exist; PRL-00 still needs `pearl_owner_id`, manual Pearl constraints, and route candidate proof |
...
| `/api/admin/pearl-owners` | `GET` | `3 7 API Contracts` | FR-27, FR-30<br>NFR-S7 | Planned: `backend/src/app.ts`<br>`backend/src/modules/admin/pearlOwners.service.ts` | Planned: `TC-PRL-API-01`<br>`TC-PRL-SEC-01` | NOT STARTED | Planned admin-only lookup for selecting an existing PearlOwner |
| `/api/admin/pearl-owners` | `POST` | `3 7 API Contracts` | FR-30<br>NFR-S7 | Planned: `backend/src/app.ts`<br>`backend/src/modules/admin/pearlOwners.service.ts` | Planned: `TC-PRL-API-01`<br>`TC-PRL-SEC-01` | NOT STARTED | Planned admin-only create flow for PearlOwner records |
| `/api/admin/pearls` | `POST` | `3 7 API Contracts` | FR-27, FR-28, FR-29, FR-30, FR-31<br>NFR-S7 | Planned: `backend/src/app.ts`<br>`backend/src/modules/admin/pearls.service.ts`<br>`backend/src/modules/gems/gems.repo.ts` | Planned: `TC-PRL-API-02`<br>`TC-PRL-API-03`<br>`TC-PRL-API-04`<br>`TC-PRL-SEC-01` | NOT STARTED | Planned admin-only create endpoint; successful Pearls must be active manual route candidates |
...
These are planned validation IDs for PRL-00 implementation tickets. They are not evidence of shipped feature code yet.

| Test Case ID | Coverage Summary | Planned Evidence | Linked FR / NFR / GH |
| --- | --- | --- | --- |
| `TC-PRL-DB-01` | PearlOwner schema and manual Pearl relationship are migrated with required constraints | Planned DB migration/schema test | FR-29, FR-30 |
| `TC-PRL-API-01` | Admin can search/create PearlOwner records and non-admin requests are blocked | Planned backend API test | FR-30, NFR-S7, `GH-PRL-02`, `GH-PRL-05` |
| `TC-PRL-API-02` | Add Pearl rejects missing required fields, invalid coordinates, and themes outside the MVP whitelist | Planned backend API test | FR-28, FR-29, `GH-PRL-03` |
| `TC-PRL-API-03` | Add Pearl saves a valid Pearl linked to exactly one existing PearlOwner | Planned backend API test | FR-27, FR-28, FR-30, `GH-PRL-01` |
| `TC-PRL-API-04` | Add Pearl response marks the new Pearl as an active route candidate | Planned backend API/integration test | FR-31, `GH-PRL-04` |
| `TC-PRL-INT-01` | Existing route generation candidate loading can include the newly created active manual Pearl | Planned backend integration test | FR-31, `GH-PRL-01`, `GH-PRL-04` |
