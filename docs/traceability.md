...
| `GH-PRL-01` Admin adds a Pearl with an existing PearlOwner | FR-27, FR-28, FR-30, FR-31 | Backend Add Pearl API create flow, PearlOwner relationship, and route candidate response exist; frontend form remains planned | `TC-PRL-API-03`<br>`TC-PRL-API-04`<br>Planned: `TC-PRL-UI-01`<br>`TC-PRL-INT-01` | PARTIAL | Backend foundation proves save contract and route-eligible flag; full UI flow remains Prompt 2 |
| `GH-PRL-02` Admin creates a PearlOwner during Add Pearl | FR-30 | PearlOwner create API exists; UI select/create flow remains planned | `TC-PRL-API-01`<br>Planned: `TC-PRL-UI-03` | PARTIAL | New owner persistence target is available before frontend integration |
| `GH-PRL-03` Invalid or missing Pearl fields block save | FR-27, FR-28, FR-29, FR-30 | Backend validation rejects missing fields, invalid theme, invalid coordinates, and invalid PearlOwner IDs | `TC-PRL-API-02`<br>Planned: `TC-PRL-UI-02` | PARTIAL | Backend rejects invalid input before save; frontend inline validation remains Prompt 2 |
| `GH-PRL-04` New Pearl is selectable for route generation | FR-29, FR-31 | Backend persists Pearls as active manual `gems`; route candidate query already loads active gems by theme | `TC-PRL-API-04`<br>`TC-PRL-INT-01` | PARTIAL | Code path is wired for backend route eligibility; full UI-driven route proof remains Prompt 2 |
| `GH-PRL-05` Add Pearl remains admin-only | FR-27 | Backend Add Pearl and PearlOwner endpoints require the existing admin session | `TC-PRL-SEC-01` | PARTIAL | Backend enforces admin-only scope without adding login/account implementation; frontend route guard remains Prompt 2 |
...
| `frontend/src/routes/AdminAddPearlPage.tsx` | `3 5 UI Design`<br>`3 6 UI ↔ API Mapping` | FR-27, FR-28, FR-29, FR-30, FR-31 | `/api/admin/pearl-owners`, `/api/admin/pearls` | Planned: `TC-PRL-UI-01`<br>`TC-PRL-UI-02`<br>`TC-PRL-SEC-01` | NOT STARTED | Planned admin-only route/shell for Add Pearl; login implementation remains out of scope |
| `frontend/src/components/admin/AddPearlForm.tsx` | `3 5 UI Design`<br>`3 6 UI ↔ API Mapping` | FR-27, FR-28, FR-29, FR-31 | `/api/admin/pearls` | Planned: `TC-PRL-UI-01`<br>`TC-PRL-UI-02` | NOT STARTED | Planned required field, theme, coordinate, submit, error, and success states |
| `frontend/src/components/admin/PearlOwnerSelect.tsx` | `3 5 UI Design`<br>`3 6 UI ↔ API Mapping` | FR-30 | `/api/admin/pearl-owners` | Planned: `TC-PRL-UI-03` | NOT STARTED | Planned select-or-create UI so PearlOwner is persisted as a relationship |
...
## 14. Admin Pearl Management Planned Test Cases

These IDs track PRL-00 implementation evidence. Backend foundation evidence is now present; frontend Prompt 2 IDs remain planned.
...
| `TC-PRL-UI-01` | Authenticated admin can complete the Add Pearl happy path with an existing PearlOwner | Planned frontend test | FR-27, FR-28, FR-31, `GH-PRL-01` |
| `TC-PRL-UI-02` | Add Pearl form shows inline validation for missing fields and invalid theme before save | Planned frontend test | FR-28, FR-29, `GH-PRL-03` |
| `TC-PRL-UI-03` | Add Pearl form supports selecting an existing PearlOwner and creating a new one | Planned frontend test | FR-30, `GH-PRL-02` |
| `TC-PRL-SEC-01` | Add Pearl UI/API remain inaccessible without a valid admin session | Planned backend and frontend auth guard tests | FR-27, NFR-S7, `GH-PRL-05` |
